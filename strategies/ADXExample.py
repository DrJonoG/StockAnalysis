...
def get_historical_data():
    ...
    api_key = open('../config/api.conf').readline().rstrip()

    api_url = f'https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY_EXTENDED&symbol={symbol}&interval=1min&slice=year1month2&apikey={api_key}'
    ...
    sliceDF = pd.read_csv(StringIO(rawData.text))
...
print('Profit gained from the ADX strategy by investing $100k : {}'.format(total_investment_ret))
print('Profit percentage of the ADX strategy : {}%'.format(profit_percentage))

print(adx_investment_ret_df)
